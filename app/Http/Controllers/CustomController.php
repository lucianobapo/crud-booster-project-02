...
class CustomController extends \crocodicstudio\crudbooster\controllers\CBController
{
    public $uuid_field = null;
    ...
    public function cbCustomInit(){
        if (CRUDBooster::isSuperadmin()){
            array_unshift($this->col,["label"=>"Owner","name"=>"owner_id"]);
//            array_unshift($this->col,["label"=>"ID","name"=>"id"]);
        }

    }

    public function hook_custom_query_index(&$query)
    {
        $this->hook_query_index($query);

        $me = CRUDBooster::me();
        if(!empty($me->owner_id) && !CRUDBooster::isSuperadmin())
            $query->where($this->table.'.owner_id',$me->owner_id);

    }

    public function hook_custom_before_add(&$arr)
    {
        $this->hook_before_add($arr);

        $me = CRUDBooster::me();
        if(!empty($me->owner_id)) $arr['owner_id'] = $me->owner_id;


        if (!is_null($this->uuid_field))
            if($this->uuid_field===true)
                $arr[$this->primary_key] = Uuid::uuid4();
            else
                $arr[$this->uuid_field] = Uuid::uuid4();
    }
    ...
    public function getIndex()
    {
        ...
        $this->hook_custom_query_index($result);
    ...
    public function postAddSave()
    {
        ...
        $this->hook_custom_before_add($this->arr);
            ...
            if ($ro['type'] == 'child') {
                ...
                for ($i = 0; $i <= $count_input_data; $i++) {
                    ...
                    if(!empty($column_data)){
                        $column_data[$fk] = $id;
                        if (Schema::hasColumn($childtable, 'created_at')) {
            ...
            if ($ro['type'] == 'child') {
                ...
                for ($i = 0; $i <= $count_input_data; $i++) {
                    ...
                    if(!empty($column_data)){
                        ...
                        if (Schema::hasColumn($childtable, 'updated_at')) {
                            $column_data['updated_at'] = date('Y-m-d H:i:s');
                        }
